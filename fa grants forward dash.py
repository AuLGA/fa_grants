...
def read_data(): 
    ...
    out_columns = ['Jurisdiction', 'LGA', 'ACLG', 
    'ERP_2024', 'Grant_2024', 
    'ERP_2025', 'Grant_2025', 'Grant_base_2025',
    'ERP_2026', 'Grant_2026', 'Grant_base_2026',
    'ERP_2027', 'Grant_2027', 'Grant_base_2027', 
    'ERP_2028', 'Grant_2028', 'Grant_base_2028', 'UID'] 
    ...
    return budget_grants, out_columns, nsw_grants_base, vic_grants_base, qld_grants_base, sa_grants_base, wa_grants_base, tas_grants_base, nt_grants_base

def correct_delta_vic(sub_vic, year): 
    ...
    while np.any((sub_vic["delta"].round(2)<2)|(sub_vic["delta"].round(2)>10)):
        ...
        sub_vic["alloc"] -= sub_vic["g"] * sub_vic[f"Grant_{year-1}"] 
        remainder = total_funding - sub_vic["alloc"].sum() 
        sub_vic["alloc"] += sub_vic[f"Funding Gap_{year}"] * remainder / total_funding_gap 
        sub_vic["delta"] = (sub_vic["alloc"] - sub_vic[f"Grant_{year-1}"]) / sub_vic[f"Grant_{year-1}"] * 100
    return sub_vic 

def correct_delta_sa(sub_sa, year): 
    ...
    while np.any((sub_sa["delta"].round(2)<-15)|(sub_sa["delta"].round(2)>30)):
        ...
        sub_sa["alloc"] -= sub_sa["g"] * sub_sa[f"Grant_{year-1}"] 
        remainder = total_funding - sub_sa["alloc"].sum() 
        sub_sa["alloc"] += sub_sa[f"Funding Gap_{year}"] * remainder / total_funding_gap 
        sub_sa["delta"] = (sub_sa["alloc"] - sub_sa[f"Grant_{year-1}"]) / sub_sa[f"Grant_{year-1}"] * 100
    return sub_sa 
 
def correct_delta_wa(sub_wa, year): 
    ...
    while np.any((sub_wa["delta"].round(2)<-60)):
        ...
        sub_wa["alloc"] -= sub_wa["g"] * sub_wa[f"Grant_{year-1}"] 
        remainder = total_funding - sub_wa["alloc"].sum() 
        sub_wa["alloc"] += sub_wa[f"Funding Gap_{year}"] * remainder / total_funding_gap 
        sub_wa["delta"] = (sub_wa["alloc"] - sub_wa[f"Grant_{year-1}"]) / sub_wa[f"Grant_{year-1}"] * 100
    return sub_wa 
 
def correct_delta_tas(sub_tas, year): 
    ...
    while np.any((sub_tas["delta"].round(2)<-5)|(sub_tas["delta"].round(2)>10)):
        ...
        sub_tas["alloc"] -= sub_tas["g"] * sub_tas[f"Grant_{year-1}"] 
        remainder = total_funding - sub_tas["alloc"].sum() 
        sub_tas["alloc"] += sub_tas[f"Funding Gap_{year}"] * remainder / total_funding_gap 
        sub_tas["delta"] = (sub_tas["alloc"] - sub_tas[f"Grant_{year-1}"]) / sub_tas[f"Grant_{year-1}"] * 100
    return sub_tas

def correct_delta_nt(sub_nt, year): 
    ...
    while np.any((sub_nt["delta"].round(2)<-5)):
        ...
        sub_nt["alloc"] -= sub_nt["g"] * sub_nt[f"Grant_{year-1}"] 
...
def simulate(per_capita_minimum, per_capita_minimum_base):
    ...
    with st.status("Running simulation...", expanded=True) as status:
        ...
        budget_grants, out_columns, nsw_grants_base, vic_grants_base, qld_grants_base, sa_grants_base, wa_grants_base, tas_grants_base, nt_grants_base = read_data()

        master = pd.DataFrame(columns=out_columns) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, nsw_grants[out_columns]]) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, vic_grants[out_columns]]) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, qld_grants[out_columns]]) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, sa_grants[out_columns]]) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, wa_grants[out_columns]]) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, tas_grants[out_columns]]) 
        ...
        for i in range(1000):
            ...
            master = pd.concat([master, nt_grants[out_columns]]) 
...
with st.sidebar:
    with st.form("controls"):
        ...
        st.write("Adjust the parameters below to simulate different scenarios.")
        
        per_capita_minimum = st.number_input(
            label="New Scenario Per Capita Minimum Percentage",
            min_value=0.0,
            max_value=100.0
        )

        per_capita_minimum_base = st.number_input(
            label="Base Per Capita Minimum Percentage",
            min_value=0.0,
            max_value=100.0
        )
        ...
        if submitted:
            simulate(per_capita_minimum, per_capita_minimum_base)
