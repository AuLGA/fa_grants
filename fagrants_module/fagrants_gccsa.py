import pandas as pd
import numpy as np
from numpy.random import normal, seed
import itertools

pd.options.mode.chained_assignment = None

from time import time

from multiprocessing import Pool


class fagrants_model:
    """
    A class to model the Australian Financial Assistance Grants system.

    This class provides methods to run simulations for different states in Australia,
    generating financial assistance grants based on population and budget data.
    """

    def __init__(
        self,
        data_path="./Data/FA Grants Tables - Python.xlsx",
        per_capita_minimum=0.2,
        per_capita_minimum_base=0.3,
    ):
        """
        Initialize the fagrants class with data path and constants.
        :param data_path: Path to the Excel file containing financial assistance grants data.
        :param per_capita_minimum: Minimum per capita grant percentage.
        :param per_capita_minimum_base: Base minimum per capita grant percentage.
        """
        # ## Controls and constants

        self.data_path = data_path
        self.per_capita_minimum = per_capita_minimum
        self.per_capita_minimum_base = per_capita_minimum_base

        self.pop_30_jun = pd.read_excel(
            self.data_path, sheet_name="Population", skiprows=21, nrows=7
        )
        self.pop_30_jun.set_index("Population by GCCSA", inplace=True)
        self.pop_30_jun = self.pop_30_jun

        self.budget_grants = pd.read_excel(self.data_path, sheet_name="Grants")
        self.budget_grants.bfill(inplace=True)
        self.budget_grants.set_index(
            "Financial Assistance Grant program (million)", inplace=True
        )
        self.budget_grants = self.budget_grants.loc[
            ["2025-26", "2026-27", "2027-28", "2028-29"]
        ]
        self.budget_grants = self.budget_grants * 1e6

        self.nsw_grants_base = pd.read_excel(self.data_path, sheet_name="New South Wales")

        self.vic_grants_base = pd.read_excel(self.data_path, sheet_name="Victoria")
        self.vic_grants_base["Grant_base_2024"] = self.vic_grants_base["Grant_2024"]

        self.qld_grants_base = pd.read_excel(self.data_path, sheet_name="Queensland")

        self.sa_grants_base = pd.read_excel(self.data_path, sheet_name="South Australia")
        self.sa_grants_base["Grant_base_2024"] = self.sa_grants_base["Grant_2024"]

        self.wa_grants_base = pd.read_excel(self.data_path, sheet_name="Western Australia")
        self.wa_grants_base["Grant_base_2024"] = self.wa_grants_base["Grant_2024"]

        self.tas_grants_base = pd.read_excel(self.data_path, sheet_name="Tasmania")
        self.tas_grants_base["Grant_base_2024"] = self.tas_grants_base["Grant_2024"]

        self.nt_grants_base = pd.read_excel(self.data_path, sheet_name="Northern Territory")
        self.nt_grants_base["Grant_base_2024"] = self.nt_grants_base["Grant_2024"]
        
        iters = [
            [self.nsw_grants_base, self.vic_grants_base, self.qld_grants_base, self.sa_grants_base, self.wa_grants_base, self.tas_grants_base, self.nt_grants_base],
            [2025, 2026, 2027, 2028]
        ]

        for df, year in itertools.product(*iters):
            mask = df["GCCSA"].str.contains("Rest of")
            
            df_gcc = df[~mask]
            df_rs = df[mask]

            df[f"ERP_{year}"] = 0.0

            df.loc[mask, f"ERP_{year}"] = df_rs["ERP_2024"] * self.pop_30_jun[df_rs["GCCSA"].iloc[0]][year] / df_rs["ERP_2024"].sum()
            df.loc[~mask, f"ERP_{year}"] = df_gcc["ERP_2024"] * self.pop_30_jun[df_gcc["GCCSA"].iloc[0]][year] / df_gcc["ERP_2024"].sum()

            df[f"ERP_{year}"] = df[f"ERP_{year}"].round(0)


    def init_pool_procs(self):
        seed()

    def run(self, out_path=None):
        """
        Run the financial assistance grants simulation.
        """
        pool = Pool(initializer=self.init_pool_procs)

        nsw_grants_master = pd.concat(
            pool.map_async(self.run_nsw, range(1000)).get(), axis=0, ignore_index=True
        )
        vic_grants_master = pd.concat(
            pool.map_async(self.run_vic, range(1000)).get(), axis=0, ignore_index=True
        )
        qld_grants_master = pd.concat(
            pool.map_async(self.run_qld, range(1000)).get(), axis=0, ignore_index=True
        )
        sa_grants_master = pd.concat(
            pool.map_async(self.run_sa, range(1000)).get(), axis=0, ignore_index=True
        )
        wa_grants_master = pd.concat(
            pool.map_async(self.run_wa, range(1000)).get(), axis=0, ignore_index=True
        )
        tas_grants_master = pd.concat(
            pool.map_async(self.run_tas, range(1000)).get(), axis=0, ignore_index=True
        )
        nt_grants_master = pd.concat(
            pool.map_async(self.run_nt, range(1000)).get(), axis=0, ignore_index=True
        )

        pool.close()

        # nsw_grants_master = pd.concat([self.run_nsw(i) for i in range(1000)], axis=0, ignore_index=True)
        # vic_grants_master = pd.concat([self.run_vic(i) for i in range(1000)], axis=0, ignore_index=True)
        # qld_grants_master = pd.concat([self.run_qld(i) for i in range(1000)], axis=0, ignore_index=True)
        # sa_grants_master = pd.concat([self.run_sa(i) for i in range(1000)], axis=0, ignore_index=True)
        # wa_grants_master = pd.concat([self.run_wa(i) for i in range(1000)], axis=0, ignore_index=True)
        # tas_grants_master = pd.concat([self.run_tas(i) for i in range(1000)], axis=0, ignore_index=True)
        # nt_grants_master = pd.concat([self.run_nt(i) for i in range(1000)], axis=0, ignore_index=True)

        out_columns = [
            "Jurisdiction",
            "LGA",
            "ACLG",
            "UID",
            "ERP_2024",
            "Grant_2024",
            "Min Grant Council_2025",
            "ERP_2025",
            "Grant_base_2025",
            "Grant_2025",
            "Min Grant Council_2026",
            "ERP_2026",
            "Grant_base_2026",
            "Grant_2026",
            "Min Grant Council_2027",
            "ERP_2027",
            "Grant_base_2027",
            "Grant_2027",
            "Min Grant Council_2028",
            "ERP_2028",
            "Grant_base_2028",
            "Grant_2028",
        ]

        grants_master = pd.concat(
            [
                nsw_grants_master[out_columns],
                vic_grants_master[out_columns],
                qld_grants_master[out_columns],
                sa_grants_master[out_columns],
                wa_grants_master[out_columns],
                tas_grants_master[out_columns],
                nt_grants_master[out_columns],
            ],
            axis=0,
            ignore_index=True,
        )

        if out_path:
            grants_master.to_csv(out_path, index=False)

        return grants_master

    # ## New South Wales ✅

    # Absense of reliable documentation means that we use the same approach as used in Queensland's modelling.

    def run_nsw(self, i):
        nsw_grants = self.nsw_grants_base.copy()
        nsw_grants["UID"] = (
            nsw_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["NSW"][f"{year}-{(year+1)%1000}"]
                / nsw_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = nsw_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["NSW"][f"{year}-{(year+1)%1000}"]
                / nsw_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = nsw_grants[f"ERP_{year}"] * min_per_capita_grant_base
            # non_min = nsw_grants[f"Scaled Gap_{year-1}"].round(4) == 0.0

            nsw_grants[f"Scaled Gap_{year}"] = nsw_grants[
                f"Scaled Gap_{year-1}"
            ] * normal(
                1, 0.1, size=nsw_grants.shape[0]
            )  # #* non_min
            nsw_grants[f"Scaled Gap_{year}"] = (
                nsw_grants[f"Scaled Gap_{year}"]
                / nsw_grants[f"Scaled Gap_{year}"].sum()
            )
            # New case
            raw_alloc = nsw_grants[f"Scaled Gap_{year}"] * (
                self.budget_grants["NSW"][f"{year}-{(year+1)%1000}"] - min_grant.sum()
            )
            nsw_grants[f"Grant_{year}"] = min_grant + raw_alloc
            # Base case
            raw_alloc = nsw_grants[f"Scaled Gap_{year}"] * (
                self.budget_grants["NSW"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            nsw_grants[f"Grant_base_{year}"] = min_grant_base + raw_alloc

            nsw_grants[f"Min Grant Council_{year}"] = ["NA"] * nsw_grants.shape[0]

        return nsw_grants

    # ## Victoria ✅

    # Victoria OLG has a cap/collar of [2%, 10%] for non minimum grant councils.

    def correct_delta_vic(self, sub_vic, year, base=False):
        total_funding_gap = (sub_vic[f"Funding Gap_{year}"] * sub_vic["deficit"]).sum()
        total_funding = sub_vic["alloc"].sum()

        comparison_column = "Grant_base_" if base else "Grant_"

        i = 0

        while (
            (i < 100)
            & (
                np.any(
                    (sub_vic["delta"].round(2) < 2) | (sub_vic["delta"].round(2) > 10)
                )
            )
        ) | ((i >= 100) & np.any(sub_vic["delta"].round(2) < 2)):
            sub_vic["g"] = sub_vic["delta"].apply(
                lambda x: (x - 2) / 100 if x < 2 else ((x - 10) / 100 if x > 10 else 0)
            )

            sub_vic["alloc"] -= sub_vic["g"] * sub_vic[f"{comparison_column}{year - 1}"]

            remainder = total_funding - sub_vic["alloc"].sum()

            sub_vic["alloc"] += (
                sub_vic[f"Funding Gap_{year}"] * remainder / total_funding_gap
            )

            sub_vic["delta"] = (
                (sub_vic["alloc"] - sub_vic[f"{comparison_column}{year - 1}"])
                / sub_vic[f"{comparison_column}{year - 1}"]
                * 100
            )

            i += 1

        return sub_vic

    def run_vic(self, i):
        vic_grants = self.vic_grants_base.copy()
        vic_grants["UID"] = (
            vic_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["VIC"][f"{year}-{(year+1)%1000}"]
                / vic_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = vic_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["VIC"][f"{year}-{(year+1)%1000}"]
                / vic_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = vic_grants[f"ERP_{year}"] * min_per_capita_grant_base
            vic_grants[f"Funding Gap_{year}"] = vic_grants[
                f"Funding Gap_{year-1}"
            ] * normal(1, 0.1, size=vic_grants.shape[0])
            vic_grants["deficit"] = vic_grants[f"Funding Gap_{year}"] > 0
            total_deficit = (
                vic_grants[f"Funding Gap_{year}"] * vic_grants["deficit"]
            ).sum()
            vic_grants[f"Min Grant Council_{year}"] = ~vic_grants["deficit"]
            # New case
            raw_alloc = (
                vic_grants[f"Funding Gap_{year}"]
                * vic_grants["deficit"]
                / total_deficit
            ) * (self.budget_grants["VIC"][f"{year}-{(year+1)%1000}"] - min_grant.sum())
            vic_grants["alloc"] = min_grant + raw_alloc
            vic_grants["delta"] = (
                (vic_grants["alloc"] - vic_grants[f"Grant_{year-1}"])
                / vic_grants[f"Grant_{year-1}"]
                * 100
            )
            non_min_vic = vic_grants[
                vic_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            vic_grants.combine_first(self.correct_delta_vic(non_min_vic, year))
            vic_grants[f"Grant_{year}"] = vic_grants["alloc"]

            # Base case
            raw_alloc = (
                vic_grants[f"Funding Gap_{year}"]
                * vic_grants["deficit"]
                / total_deficit
            ) * (
                self.budget_grants["VIC"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            vic_grants["alloc"] = min_grant_base + raw_alloc
            vic_grants["delta"] = (
                (vic_grants["alloc"] - vic_grants[f"Grant_base_{year-1}"])
                / vic_grants[f"Grant_base_{year-1}"]
                * 100
            )
            non_min_vic = vic_grants[
                vic_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            vic_grants.combine_first(
                self.correct_delta_vic(non_min_vic, year, base=True)
            )
            vic_grants[f"Grant_base_{year}"] = vic_grants["alloc"]

        return vic_grants

    # ## Queensland ✅

    # Queensland does not release their "Funding Gap" variable. Hence, an analogue is reverse engineered from the gap between grant entitlement and minimum grant. Adjustments and normalizations applied to this should, in principle, give fairly robust numbers, especially at the high level.
    #
    # There's no cap/collar and minimum grant eligibility is based on population > 80,000.

    def run_qld(self, i):
        qld_grants = self.qld_grants_base.copy()
        qld_grants["UID"] = (
            qld_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["QLD"][f"{year}-{(year+1)%1000}"]
                / qld_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = qld_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["QLD"][f"{year}-{(year+1)%1000}"]
                / qld_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = qld_grants[f"ERP_{year}"] * min_per_capita_grant_base
            non_min = qld_grants[f"ERP_{year}"] <= 80000
            qld_grants[f"Scaled Gap_{year}"] = qld_grants[
                f"Scaled Gap_{year-1}"
            ] * normal(
                1, 0.1, size=qld_grants.shape[0]
            )  # * non_min
            qld_grants[f"Scaled Gap_{year}"] = (
                qld_grants[f"Scaled Gap_{year}"]
                / qld_grants[f"Scaled Gap_{year}"].sum()
            )

            qld_grants[f"Min Grant Council_{year}"] = ~non_min
            # New case
            raw_alloc = qld_grants[f"Scaled Gap_{year}"] * (
                self.budget_grants["QLD"][f"{year}-{(year+1)%1000}"] - min_grant.sum()
            )
            qld_grants[f"Grant_{year}"] = min_grant + raw_alloc
            # Base case
            raw_alloc = qld_grants[f"Scaled Gap_{year}"] * (
                self.budget_grants["QLD"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            qld_grants[f"Grant_base_{year}"] = min_grant_base + raw_alloc
        return qld_grants

    # ## South Australia ✅

    # South Australia OLG has a cap/collar of [-15%, 30%] for councils.

    def correct_delta_sa(self, sub_sa, year, base=False):
        total_funding_gap = (sub_sa[f"Funding Gap_{year}"] * sub_sa["deficit"]).sum()
        total_funding = sub_sa["alloc"].sum()

        comparison_column = "Grant_base_" if base else "Grant_"

        i = 0

        # while np.any((sub_sa["delta"].round(2)<-15)|(sub_sa["delta"].round(2)>30)):
        while (
            (i < 100)
            & (
                np.any(
                    (sub_sa["delta"].round(2) < -15) | (sub_sa["delta"].round(2) > 30)
                )
            )
        ) | ((i >= 100) & np.any(sub_sa["delta"].round(2) < -15)):

            sub_sa["g"] = sub_sa["delta"].apply(
                lambda x: (
                    (x - (-15)) / 100 if x < -15 else ((x - 30) / 100 if x > 30 else 0)
                )
            )

            sub_sa["alloc"] -= sub_sa["g"] * sub_sa[f"{comparison_column}{year-1}"]

            remainder = total_funding - sub_sa["alloc"].sum()

            sub_sa["alloc"] += (
                sub_sa[f"Funding Gap_{year}"] * remainder / total_funding_gap
            )

            sub_sa["delta"] = (
                (sub_sa["alloc"] - sub_sa[f"{comparison_column}{year-1}"])
                / sub_sa[f"{comparison_column}{year-1}"]
                * 100
            )

            i += 1

        return sub_sa

    def run_sa(self, i):
        sa_grants = self.sa_grants_base.copy()
        sa_grants["UID"] = (
            sa_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["SA"][f"{year}-{(year+1)%1000}"]
                / sa_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = sa_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["SA"][f"{year}-{(year+1)%1000}"]
                / sa_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = sa_grants[f"ERP_{year}"] * min_per_capita_grant_base
            sa_grants[f"Funding Gap_{year}"] = sa_grants[
                f"Funding Gap_{year-1}"
            ] * normal(1, 0.1, size=sa_grants.shape[0])
            sa_grants["deficit"] = sa_grants[f"Funding Gap_{year}"] > 0
            total_deficit = (
                sa_grants[f"Funding Gap_{year}"] * sa_grants["deficit"]
            ).sum()

            sa_grants[f"Min Grant Council_{year}"] = ~sa_grants["deficit"]

            # New case
            raw_alloc = (
                sa_grants[f"Funding Gap_{year}"] * sa_grants["deficit"] / total_deficit
            ) * (self.budget_grants["SA"][f"{year}-{(year+1)%1000}"] - min_grant.sum())
            sa_grants["alloc"] = min_grant + raw_alloc
            sa_grants["delta"] = (
                (sa_grants["alloc"] - sa_grants[f"Grant_{year-1}"])
                / sa_grants[f"Grant_{year-1}"]
                * 100
            )
            non_min_sa = sa_grants[
                sa_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            sa_grants.combine_first(self.correct_delta_sa(non_min_sa, year))
            sa_grants[f"Grant_{year}"] = sa_grants["alloc"]
            # Base case
            raw_alloc = (
                sa_grants[f"Funding Gap_{year}"] * sa_grants["deficit"] / total_deficit
            ) * (
                self.budget_grants["SA"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            sa_grants["alloc"] = min_grant_base + raw_alloc
            sa_grants["delta"] = (
                (sa_grants["alloc"] - sa_grants[f"Grant_base_{year-1}"])
                / sa_grants[f"Grant_base_{year-1}"]
                * 100
            )
            non_min_sa = sa_grants[
                sa_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            sa_grants.combine_first(self.correct_delta_sa(non_min_sa, year, base=True))
            sa_grants[f"Grant_base_{year}"] = sa_grants["alloc"]

        return sa_grants

    # ## Western Australia ✅

    # WA State Grants Commission has no ceiling on change, but claims to limit year on year decrease. No explicit floor is given, but the biggest 2023-24 to 2024-25 decrease was about -60%. So, that's the floor we'll use.

    def correct_delta_wa(self, sub_wa, year, base=False):
        total_funding_gap = (sub_wa[f"Funding Gap_{year}"] * sub_wa["deficit"]).sum()
        total_funding = sub_wa["alloc"].sum()
        comparison_column = "Grant_base_" if base else "Grant_"

        while np.any((sub_wa["delta"].round(2) < -60)):
            sub_wa["g"] = sub_wa["delta"].apply(
                lambda x: (x - (-60)) / 100 if x < -60 else 0
            )
            sub_wa["alloc"] -= sub_wa["g"] * sub_wa[f"{comparison_column}{year-1}"]
            remainder = total_funding - sub_wa["alloc"].sum()
            sub_wa["alloc"] += (
                sub_wa[f"Funding Gap_{year}"] * remainder / total_funding_gap
            )
            sub_wa["delta"] = (
                (sub_wa["alloc"] - sub_wa[f"{comparison_column}{year-1}"])
                / sub_wa[f"{comparison_column}{year-1}"]
                * 100
            )
        return sub_wa

    def run_wa(self, i):
        wa_grants = self.wa_grants_base.copy()
        wa_grants["UID"] = (
            wa_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["WA"][f"{year}-{(year+1)%1000}"]
                / wa_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = wa_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["WA"][f"{year}-{(year+1)%1000}"]
                / wa_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = wa_grants[f"ERP_{year}"] * min_per_capita_grant_base
            wa_grants[f"Funding Gap_{year}"] = wa_grants[
                f"Funding Gap_{year-1}"
            ] * normal(1, 0.1, size=wa_grants.shape[0])
            wa_grants["deficit"] = wa_grants[f"Funding Gap_{year}"] > 0
            total_deficit = (
                wa_grants[f"Funding Gap_{year}"] * wa_grants["deficit"]
            ).sum()

            wa_grants[f"Min Grant Council_{year}"] = ~wa_grants["deficit"]

            # New case
            raw_alloc = (
                wa_grants[f"Funding Gap_{year}"] * wa_grants["deficit"] / total_deficit
            ) * (self.budget_grants["WA"][f"{year}-{(year+1)%1000}"] - min_grant.sum())
            wa_grants["alloc"] = min_grant + raw_alloc
            wa_grants["delta"] = (
                (wa_grants["alloc"] - wa_grants[f"Grant_{year-1}"])
                / wa_grants[f"Grant_{year-1}"]
                * 100
            )
            non_min_wa = wa_grants[
                wa_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            wa_grants.combine_first(self.correct_delta_wa(non_min_wa, year))
            wa_grants[f"Grant_{year}"] = wa_grants["alloc"]
            # Base case
            raw_alloc = (
                wa_grants[f"Funding Gap_{year}"] * wa_grants["deficit"] / total_deficit
            ) * (
                self.budget_grants["WA"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            wa_grants["alloc"] = min_grant_base + raw_alloc
            wa_grants["delta"] = (
                (wa_grants["alloc"] - wa_grants[f"Grant_base_{year-1}"])
                / wa_grants[f"Grant_base_{year-1}"]
                * 100
            )
            non_min_wa = wa_grants[
                wa_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            wa_grants.combine_first(self.correct_delta_wa(non_min_wa, year, base=True))
            wa_grants[f"Grant_base_{year}"] = wa_grants["alloc"]

        return wa_grants

    # ## Tasmania ✅

    # Tasmania State Grants Commission has a cap/collar of [-5%, 10%] for all councils.

    def correct_delta_tas(self, sub_tas, year, base=False):
        total_funding_gap = (sub_tas[f"Funding Gap_{year}"] * sub_tas["deficit"]).sum()
        total_funding = sub_tas["alloc"].sum()
        comparison_column = "Grant_base_" if base else "Grant_"

        while np.any(
            (sub_tas["delta"].round(2) < -5) | (sub_tas["delta"].round(2) > 10)
        ):
            sub_tas["g"] = sub_tas["delta"].apply(
                lambda x: (
                    (x - (-5)) / 100 if x < -5 else ((x - 10) / 100 if x > 10 else 0)
                )
            )
            sub_tas["alloc"] -= sub_tas["g"] * sub_tas[f"{comparison_column}{year-1}"]
            remainder = total_funding - sub_tas["alloc"].sum()
            sub_tas["alloc"] += (
                sub_tas[f"Funding Gap_{year}"] * remainder / total_funding_gap
            )
            delta_new = (
                (sub_tas["alloc"] - sub_tas[f"{comparison_column}{year-1}"])
                / sub_tas[f"{comparison_column}{year-1}"]
                * 100
            )
            if delta_new.round(3).equals(sub_tas["delta"].round(3)):
                sub_tas["delta"] = delta_new
                break
            sub_tas["delta"] = delta_new
        return sub_tas

    def run_tas(self, i):
        tas_grants = self.tas_grants_base.copy()
        tas_grants["UID"] = (
            tas_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["TAS"][f"{year}-{(year+1)%1000}"]
                / tas_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = tas_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["TAS"][f"{year}-{(year+1)%1000}"]
                / tas_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = tas_grants[f"ERP_{year}"] * min_per_capita_grant_base
            tas_grants[f"Funding Gap_{year}"] = tas_grants[
                f"Funding Gap_{year-1}"
            ] * normal(1, 0.1, size=tas_grants.shape[0])
            tas_grants["deficit"] = tas_grants[f"Funding Gap_{year}"] > 0
            total_deficit = (
                tas_grants[f"Funding Gap_{year}"] * tas_grants["deficit"]
            ).sum()

            tas_grants[f"Min Grant Council_{year}"] = ~tas_grants["deficit"]

            # New case
            raw_alloc = (
                tas_grants[f"Funding Gap_{year}"]
                * tas_grants["deficit"]
                / total_deficit
            ) * (self.budget_grants["TAS"][f"{year}-{(year+1)%1000}"] - min_grant.sum())
            tas_grants["alloc"] = min_grant + raw_alloc
            tas_grants["delta"] = (
                (tas_grants["alloc"] - tas_grants[f"Grant_{year-1}"])
                / tas_grants[f"Grant_{year-1}"]
                * 100
            )
            non_min_tas = tas_grants[
                tas_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            tas_grants.combine_first(self.correct_delta_tas(non_min_tas, year))
            tas_grants[f"Grant_{year}"] = tas_grants["alloc"]
            # Base case
            raw_alloc = (
                tas_grants[f"Funding Gap_{year}"]
                * tas_grants["deficit"]
                / total_deficit
            ) * (
                self.budget_grants["TAS"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            tas_grants["alloc"] = min_grant_base + raw_alloc
            tas_grants["delta"] = (
                (tas_grants["alloc"] - tas_grants[f"Grant_base_{year-1}"])
                / tas_grants[f"Grant_base_{year-1}"]
                * 100
            )
            non_min_tas = tas_grants[
                tas_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            tas_grants.combine_first(
                self.correct_delta_tas(non_min_tas, year, base=True)
            )
            tas_grants[f"Grant_base_{year}"] = tas_grants["alloc"]

        return tas_grants

    # ## Northern Territory ✅

    # Northern Territory has a collar of -5% for all councils.

    def correct_delta_nt(self, sub_nt, year, base=False):
        total_funding_gap = (sub_nt[f"Funding Gap_{year}"] * sub_nt["deficit"]).sum()
        total_funding = sub_nt["alloc"].sum()

        comparison_column = "Grant_base_" if base else "Grant_"
        while np.any((sub_nt["delta"].round(2) < -5)):
            sub_nt["g"] = sub_nt["delta"].apply(
                lambda x: (x - (-5)) / 100 if x < -5 else 0
            )
            sub_nt["alloc"] -= sub_nt["g"] * sub_nt[f"{comparison_column}{year-1}"]
            remainder = total_funding - sub_nt["alloc"].sum()
            sub_nt["alloc"] += (
                sub_nt[f"Funding Gap_{year}"] * remainder / total_funding_gap
            )
            sub_nt["delta"] = (
                (sub_nt["alloc"] - sub_nt[f"{comparison_column}{year-1}"])
                / sub_nt[f"{comparison_column}{year-1}"]
                * 100
            )
        return sub_nt

    def run_nt(self, i):
        nt_grants = self.nt_grants_base.copy()
        nt_grants["UID"] = (
            nt_grants["LGA"].str.replace(" ", "").replace("-", "") + "_" + str(i + 1)
        )
        for year in range(2025, 2029):
            min_per_capita_grant = (
                self.budget_grants["NT"][f"{year}-{(year+1)%1000}"]
                / nt_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum
            )
            min_grant = nt_grants[f"ERP_{year}"] * min_per_capita_grant
            min_per_capita_grant_base = (
                self.budget_grants["NT"][f"{year}-{(year+1)%1000}"]
                / nt_grants[f"ERP_{year}"].sum()
                * self.per_capita_minimum_base
            )
            min_grant_base = nt_grants[f"ERP_{year}"] * min_per_capita_grant_base
            nt_grants[f"Funding Gap_{year}"] = nt_grants[
                f"Funding Gap_{year-1}"
            ] * normal(1, 0.1, size=nt_grants.shape[0])
            nt_grants["deficit"] = nt_grants[f"Funding Gap_{year}"] > 0
            total_deficit = (
                nt_grants[f"Funding Gap_{year}"] * nt_grants["deficit"]
            ).sum()
            # New case
            raw_alloc = (
                nt_grants[f"Funding Gap_{year}"] * nt_grants["deficit"] / total_deficit
            ) * (self.budget_grants["NT"][f"{year}-{(year+1)%1000}"] - min_grant.sum())
            nt_grants["alloc"] = min_grant + raw_alloc
            nt_grants["delta"] = (
                (nt_grants["alloc"] - nt_grants[f"Grant_{year-1}"])
                / nt_grants[f"Grant_{year-1}"]
                * 100
            )
            nt_grants[f"Min Grant Council_{year}"] = ~nt_grants["deficit"]

            non_min_nt = nt_grants[
                nt_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            nt_grants.combine_first(self.correct_delta_nt(non_min_nt, year))
            nt_grants[f"Grant_{year}"] = nt_grants["alloc"]
            # Base case
            raw_alloc = (
                nt_grants[f"Funding Gap_{year}"] * nt_grants["deficit"] / total_deficit
            ) * (
                self.budget_grants["NT"][f"{year}-{(year+1)%1000}"]
                - min_grant_base.sum()
            )
            nt_grants["alloc"] = min_grant_base + raw_alloc
            nt_grants["delta"] = (
                (nt_grants["alloc"] - nt_grants[f"Grant_base_{year-1}"])
                / nt_grants[f"Grant_base_{year-1}"]
                * 100
            )

            non_min_nt = nt_grants[
                nt_grants["deficit"]
            ]  # [["LGA", f"Funding Gap_{year}", f"grant_{year-1}", "alloc"]]
            nt_grants.combine_first(self.correct_delta_nt(non_min_nt, year, base=True))
            nt_grants[f"Grant_base_{year}"] = nt_grants["alloc"]
        return nt_grants
